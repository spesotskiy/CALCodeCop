...
Style checks for Dynamics NAV 2017 (C/AL) object text exports. This slice parses a codeunit and runs Rule 001 (operator and comma spacing).
...
`internal/syntax` parses UTF-8 object text into an AST with a retained token stream. `internal/rules` runs Rule 001 on that tree.
